 ...
 * Transmit a command to the NMEA device.
 * The same as DEBUG_Write except this function does not call the mutex that is protecting the 
 * write buffer so the function can be used before the FreeRTOS scheduler is started
 * Inputs:
 *   id - ID of the desired NMEA channel
 *	 cmd - pointer to the command (without $ or *checksum)
 * Status:
