...
/******************************************************************************
******************************************************************************/
void transmit_radio(gps_gprmc *gprmc, gps_deci_signed *ds_robot, gps_deci_signed *ds_bouee, 
	int angleRobot, distance_deci_signed *distance, int correctionCap,int dirVent, 
	int servoVoile, int poi_actuel)
{
	int temp32,temp32_2;
	int angle1=0,angle2=0;
	
	char tx_string[RADIO_STRING_LEN];

	// time
	tx_string[0] = 0x0d;
	append(tx_string, 0x0a);
	sprintf(&tx_string[2], "%02uh%02um%02u,%03lu", gprmc->time_h, gprmc->time_mn, gprmc->time_s, gprmc->time_ds);
	append(tx_string, 0x0d);
	append(tx_string, 0x0a);
	int len = strlen(tx_string);

	////date
	sprintf(&tx_string[len], "%02u/%02u/%02u", gprmc->date_d, gprmc->date_m, gprmc->date_y);
	append(tx_string, 0x0d);
	append(tx_string, 0x0a);
	...
	usart_write_buffer_job(&usart_module, (uint8_t*)tx_string,
		RADIO_STRING_LEN);
}